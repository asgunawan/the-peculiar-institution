...
import { formatSaveDate } from "../gameLogic/saveSlotUtils";
...
function SlotRow({ index, meta, onSave, onLoad, onDelete }: SlotRowProps) {
  const occupied = meta !== null;

  function handleSave() {
    if (occupied && !window.confirm(`Overwrite slot ${index + 1} (${meta.label})? This cannot be undone.`)) return;
    onSave(index);
  }

  function handleLoad() {
    if (!occupied) return;
    if (!window.confirm(`Load slot ${index + 1} (${meta.label})? Unsaved progress will be lost.`)) return;
    onLoad(index);
  }
  ...
  return (
    <div className="slot-row">
      ...
      <button className="slot-btn slot-btn-save" onClick={handleSave} title={`Save current game to slot ${index + 1}`}>
        Save
      </button>
      <button
      ...
      <nav className="side-menu-panel">
        ...
        <div className="slot-list">
          {slotMetas.map((meta, i) => (
            <SlotRow key={i} index={i} meta={meta} onSave={onSaveToSlot} onLoad={onLoadFromSlot} onDelete={onDeleteSlot} />
          ))}
        </div>
        ...
        <p className="side-menu-hint">
          Export saves to a file you can share with the AI for scenario analysis. Import loads any exported save or a hand-crafted
          test scenario.
        </p>
        ...
        <button className="btn side-menu-btn side-menu-btn-danger" onClick={onNewGame}>
          New Game
        </button>
      </nav>
    </details>
