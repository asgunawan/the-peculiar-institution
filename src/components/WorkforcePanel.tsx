...
interface WorkforcePanelProps {
  ...
  plots: Plot[];
  onChange: (nextAssignments: Assignments) => void;
}
...
export default function WorkforcePanel({
  ...
  const activeTasks = (SEASON_TASKS[season] ?? []) as TaskName[];
  const totalAssigned = activeTasks.reduce((sum, t) => sum + (assignments[t] || 0), 0);
  const remaining = workers - totalAssigned;

  function getMaxForTask(task: TaskName): number {
    const otherSum = activeTasks.filter((t) => t !== task).reduce((sum, t) => sum + (assignments[t] || 0), 0);
    return Math.max(0, workers - otherSum);
  }

  function setTask(task: TaskName, value: number): void {
  ...
  return (
    <section className="panel workforce-panel">
      ...
      <p className="panel-meta">
        ...
        {freeCount > 0 && <span>{freeCount} hired</span>} &mdash;{" "}
        ...
        return (
          <div className="task-row" key={task}>
            <div className="task-label">
              ...
              <span className="task-hint">{taskHintNode}</span>
            </div>
      ...
      {remaining < 0 && <p className="error-msg">Over-assigned by {Math.abs(remaining)} worker(s). Reduce assignments.</p>}
    </section>
  );
}