...
interface UseSeasonAdvanceParams {
  state: GameState;
...
export function useSeasonAdvance({ state, setState, setCurrentPrice, addToast }: UseSeasonAdvanceParams) {
  const handleAdvanceSeason = useCallback(() => {
    ...
    const assignedNow = currentActiveTasks.reduce((sum, task) => sum + (state.assignments[task] || 0), 0);
